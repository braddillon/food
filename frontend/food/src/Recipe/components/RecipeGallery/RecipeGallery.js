...
class RecipeGallery extends Component {
    ...
    toggleTag = id => {
        this.setState(prevState => {
            if (prevState.activeTags.includes(id)) return { activeTags: prevState.activeTags.filter(item => item !== id) };
            else return { activeTags: [...prevState.activeTags, id] };
        });
    };
    ...
    render() {
        ...
        // Filtering
        if (this.state.searchTerm !== '') filteredRecipes = filteredRecipes.filter(id => recipes[id].name.toLowerCase().includes(this.state.searchTerm.toLowerCase()));

        if (this.state.activeTags.length > 0) {
            this.state.activeTags.forEach(tagId => {
                filteredRecipes = filteredRecipes.filter(id => recipes[id].tags.toLowerCase().includes(this.props.tags[tagId].name.toLowerCase()));
            });
        }

        // Sort recipes
        filteredRecipes = filteredRecipes.sort((a, b) => {
            if (recipes[a].slug < recipes[b].slug) {
                return -1;
            }
            if (recipes[a].slug > recipes[b].slug) {
                return 1;
        ...
        let recipeTags = filteredRecipes.map(id => <RecipeGalleryItem name={recipes[id].name} id={id} slug={recipes[id].slug} thumbnail={recipes[id].thumbnail} key={id} tags={recipes[id].tags} />);
            ...
            <div className={classes.root}>
                <Grid container spacing={4}>
                    ...
                    <Grid item sm={10} lg={11}>
                        <div className={classes.content}>{_.isEmpty(recipes) ? 'Loading' : recipeTags}</div>
                    </Grid>
                </Grid>
            </div>
        );
